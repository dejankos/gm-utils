...
impl Mvn {
    ...
    pub fn set_new_version(&self, ver: &str, project_path: PathBuf) -> Result<()> {
        ...
        self.log.info(format!("Changing mvn project version to {}", &new_version).as_str());
    ...
    fn split_current_ver(&self, project_path: &PathBuf) -> Result<Vec<String>> {
        ...
        self.log.info(format!("Current mvn project version {}", current_ver).as_str());
    ...
    fn change_version(&self, new_version: &str, project_path: &PathBuf) -> Result<()> {
        let out = Command::new("mvn")
            .args(&["-f", format!("{}/{}", project_path.to_str().unwrap(), POM_FILE).as_str(), "versions:set", format!("-DnewVersion={}", new_version).as_str()])
            .output()
        ...
        if !out_str.contains(SUCCESS) {
            return Err(
                Error::new(ErrorKind::InvalidData, "Never thought this was gonna work anyway")
            );
        }
    ...
    fn find_version(&self, project_path: &PathBuf) -> Result<String> {
        ...
        self.parse_pom_ver(c)
            .ok_or(
                Error::new(ErrorKind::InvalidData, "Failed to parse project version")
            )
    }
